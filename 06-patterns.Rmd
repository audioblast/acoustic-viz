...
### Using `format="mid-month"`

## Behind the scenes

### `radialPolygon()`

The majority of the plotting performed for cyclical plots in `SonicScrewdriver` is performed by the `radialPolygon()` function. This function can be used to plot sectors, annuli, horizon plots and irregular polygons. If you wish to customise cyclical plots beyond what the provided functions such as `dielRings()` allow then it is likely that `radialPolygon()` is the tool you need.
